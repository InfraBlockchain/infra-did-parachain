...
pub const WEIGHT_PER_SECOND: Weight = Weight::from_parts(1_000_000_000_000, u64::MAX);
...
fn expand_migrator() {
	new_test_ext().execute_with(|| {
		...
		assert!(MigrationModule::expand_migrator(RawOrigin::Signed(100).into(), acc_1, 35).is_err());
...
fn signed_extension_test() {
	...
	new_test_ext().execute_with(|| {
		...
		let call_1: <TestRuntime as system::Config>::RuntimeCall =
			RuntimeCall::MigrationModule(MigrateCall::migrate { recipients: BTreeMap::new() });
		// The call made by migrator. The bonus vectors being empty is irrelevant for this test.
		let call_2: <TestRuntime as system::Config>::RuntimeCall =
			RuntimeCall::MigrationModule(MigrateCall::give_bonuses {
		...
		for call in [&call_1, &call_2].iter() {
			assert!(signed_extension.validate(&migrator_acc_1, call, &tx_info, 20).is_ok());
			assert!(signed_extension.validate(&migrator_acc_2, call, &tx_info, 20).is_ok());
		}

		// Unregistered migrator should not pass signed extension
		for call in [&call_1, &call_2].iter() {
			assert!(signed_extension.validate(&migrator_acc_3, call, &tx_info, 20).is_err());
		}
		...
		for call in [&call_1, &call_2].iter() {
			assert!(signed_extension.validate(&migrator_acc_3, call, &tx_info, 20).is_ok());
		}

		assert_ok!(MigrationModule::remove_migrator(RawOrigin::Root.into(), migrator_acc_1));

		// Call from a previously registered but currently unregistered migrator
		for call in [&call_1, &call_2].iter() {
			assert!(signed_extension.validate(&migrator_acc_1, call, &tx_info, 20).is_err());
		}
	});
}
...
fn give_bonuses() {
	...
	new_test_ext().execute_with(|| {
		...
		// Cannot give bonuses beyond migration limit even with accounts divided between vesting and
		// swap
		assert_err!(
			MigrationModule::give_bonuses(
				RawOrigin::Signed(sender_2).into(),
...
fn swap_bonus_claims() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_eq!(MigrationModule::bonus(&recip_1).unwrap().swap_bonuses.len(), 1);
		// Vesting bonuses vector is empty
		assert!(MigrationModule::bonus(&recip_1).unwrap().vesting_bonuses.is_empty());
		...
		assert_eq!(MigrationModule::bonus(&recip_2).unwrap().swap_bonuses.len(), 2);
		assert_eq!(MigrationModule::bonus(&recip_3).unwrap().swap_bonuses.len(), 2);
		// Vesting bonuses vector is empty
		assert!(MigrationModule::bonus(&recip_2).unwrap().vesting_bonuses.is_empty());
		assert!(MigrationModule::bonus(&recip_3).unwrap().vesting_bonuses.is_empty());
		assert_eq!(recip_2_bal_1, Balances::free_balance(&recip_2));
		assert_eq!(recip_3_bal_1, Balances::free_balance(&recip_3));
		...
		assert_eq!(MigrationModule::bonus(&recip_2).unwrap().swap_bonuses.len(), 1);
		assert_eq!(MigrationModule::bonus(&recip_3).unwrap().swap_bonuses.len(), 1);
		...
		assert_eq!(MigrationModule::bonus(&recip_1).unwrap().swap_bonuses.len(), 3);
		...
		assert!(MigrationModule::bonus(&recip_1).is_none());
		assert_eq!(recip_1_bal_2 + 3 * amount_3, Balances::free_balance(&recip_1));
		assert!(MigrationModule::unlock_swap_bonus(recip_1).is_err());
	});
}
...
fn vesting_bonus_claims_1() {
	...
	new_test_ext().execute_with(|| {
		...
		assert!(MigrationModule::bonus(&recip).unwrap().swap_bonuses.is_empty());

		// Vesting bonuses vector not is empty
		assert_eq!(MigrationModule::bonus(&recip).unwrap().vesting_bonuses.len(), 1);
...
fn vesting_bonus_claims_2() {
	// Check vesting bonus. Bonus duration does not divide milestone count exactly and with and
...
fn bonus_claim_extrinsics() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(MigrationModule::bonus(&recip_1).unwrap().swap_bonuses.len(), 1);
		assert_eq!(MigrationModule::bonus(&recip_1).unwrap().vesting_bonuses.len(), 1);
		assert_eq!(MigrationModule::bonus(&recip_2).unwrap().swap_bonuses.len(), 1);
		assert_eq!(MigrationModule::bonus(&recip_2).unwrap().vesting_bonuses.len(), 1);
		...
		assert!(MigrationModule::bonus(&recip_1).unwrap().swap_bonuses.is_empty());
		assert!(MigrationModule::bonus(&recip_2).unwrap().swap_bonuses.is_empty());
		assert_eq!(MigrationModule::bonus(&recip_1).unwrap().vesting_bonuses.len(), 1);
		assert_eq!(MigrationModule::bonus(&recip_2).unwrap().vesting_bonuses.len(), 1);
