...
impl UncheckedDidKey {
	...
	pub fn new(public_key: impl Into<PublicKey>, ver_rels: VerRelType) -> Self {
		UncheckedDidKey { public_key: public_key.into(), ver_rels }
	}
...
impl TryFrom<UncheckedDidKey> for DidKey {
	...
	fn try_from(
		UncheckedDidKey { public_key, ver_rels }: UncheckedDidKey,
...
impl From<DidKey> for UncheckedDidKey {
	fn from(DidKey { public_key, ver_rels }: DidKey) -> Self {
...
mod tests {
	...
	fn encode() {
		...
		assert_eq!(VerRelType::decode(&mut &2u16.encode()[..]).unwrap(), VerRelType::ASSERTION);
