...
pub fn check_did_detail(
	...
	assert_eq!(did_detail.data().active_controller_keys, active_controller_keys);
	assert_eq!(did_detail.data().active_controllers, active_controllers);
	assert_eq!(did_detail.nonce, <Test as system::Config>::BlockNumber::from(nonce));
}
...
fn offchain_did() {
	...
	ext().execute_with(|| {
		...
		assert_noop!(
			DIDModule::new_offchain(RuntimeOrigin::signed(alice), did.clone(), doc_ref.clone()),
		...
		let OffChainDidDetails { account_id: owner, doc_ref: fetched_ref } =
		...
		assert_ok!(DIDModule::remove_offchain_did(RuntimeOrigin::signed(alice), did));
...
fn onchain_keyless_did_creation() {
	...
	ext().execute_with(|| {
		...
		assert_ok!(DIDModule::new_onchain(
			...
			did_2,
			vec![].into_iter().collect(),
			vec![Controller(did_1), controller_1, controller_2].into_iter().collect()
		));
...
fn onchain_keyed_did_creation_with_self_control() {
	...
	ext().execute_with(|| {
		...
		assert_ok!(DIDModule::new_onchain(
			RuntimeOrigin::signed(alice),
			did_1,
			vec![UncheckedDidKey::new(PublicKey::sr25519(pk_sr), VerRelType::NONE)],
			vec![].into_iter().collect()
		));
		assert!(DIDModule::is_self_controlled(&did_1));
		...
		assert_ok!(DIDModule::new_onchain(
			RuntimeOrigin::signed(alice),
			did_2,
			vec![UncheckedDidKey::new(PublicKey::ed25519(pk_ed), VerRelType::NONE)],
		...
		assert_ok!(DIDModule::new_onchain(
			RuntimeOrigin::signed(alice),
			did_4,
			vec![UncheckedDidKey::new(PublicKey::x25519(pk_ed), VerRelType::NONE)],
		...
		for pk in vec![PublicKey::sr25519(pk_sr), PublicKey::ed25519(pk_ed), pk_secp.clone()] {
			for vr in vec![
				VerRelType::AUTHENTICATION,
				VerRelType::ASSERTION,
				VerRelType::CAPABILITY_INVOCATION,
		...
		for (did, pk, vr) in vec![
			([72; Did::BYTE_SIZE], PublicKey::sr25519(pk_sr), VerRelType::ASSERTION),
			([73; Did::BYTE_SIZE], PublicKey::ed25519(pk_ed), VerRelType::ASSERTION),
			([74; Did::BYTE_SIZE], pk_secp.clone(), VerRelType::ASSERTION),
			([75; Did::BYTE_SIZE], PublicKey::sr25519(pk_sr), VerRelType::AUTHENTICATION),
			([76; Did::BYTE_SIZE], PublicKey::ed25519(pk_ed), VerRelType::AUTHENTICATION),
			([77; Did::BYTE_SIZE], pk_secp.clone(), VerRelType::AUTHENTICATION),
		] {
...
fn onchain_keyed_did_creation_with_and_without_self_control() {
	...
	ext().execute_with(|| {
		...
		assert_ok!(DIDModule::new_onchain(
			RuntimeOrigin::signed(alice),
			did_1,
			vec![UncheckedDidKey::new(PublicKey::sr25519(pk_sr), VerRelType::AUTHENTICATION)],
			vec![controller_1].into_iter().collect()
		));
		assert!(!DIDModule::is_self_controlled(&did_1));
		assert!(DIDModule::is_controller(&did_1, &controller_1));
		...
		assert_ok!(DIDModule::new_onchain(
			RuntimeOrigin::signed(alice),
			did_2,
			vec![UncheckedDidKey::new(PublicKey::ed25519(pk_ed), VerRelType::ASSERTION)],
		...
		assert_ok!(DIDModule::new_onchain(
			RuntimeOrigin::signed(alice),
			did_3,
			vec![UncheckedDidKey::new(PublicKey::x25519(pk_ed), VerRelType::KEY_AGREEMENT)],
	...
	ext().execute_with(|| {
		...
		assert_noop!(
			DIDModule::add_keys(
				RuntimeOrigin::signed(alice),
				add_keys,
				DidSignature { did: Controller(did_1), key_id: 1u32.into(), sig }
			),
		...
		let add_keys = AddKeys { did: did_1, keys: vec![], nonce: 5 };
		let sig = SigValue::sr25519(&add_keys.to_state_change().encode(), &pair_sr_1);
		assert_noop!(
			DIDModule::add_keys(
				RuntimeOrigin::signed(alice),
		...
		assert_noop!(
			DIDModule::add_keys(
				RuntimeOrigin::signed(alice),
				add_keys,
				DidSignature { did: Controller(did_2), key_id: 1u32.into(), sig }
			),
			Error::<Test>::OnlyControllerCanUpdate
		);
		...
		assert_noop!(
			DIDModule::add_keys(
				RuntimeOrigin::signed(alice),
				add_keys,
				DidSignature { did: Controller(did_1), key_id: 2u32.into(), sig }
			),
			Error::<Test>::InvalidSignature
		);
		...
		for add in vec![VerRelType::NONE, VerRelType::KEY_AGREEMENT] {
			...
			.map(|val| val | add)
			{
				...
				let add_keys = AddKeys { did: did_2, keys: vec![key], nonce: 5 + 1 };
				let sig = SigValue::sr25519(&add_keys.to_state_change().encode(), &pair_sr_1);
				assert_noop!(
					DIDModule::add_keys(
						RuntimeOrigin::signed(alice),
		...
		let add_keys = AddKeys {
			...
			keys: vec![
				...
				UncheckedDidKey::new(pk_secp_2, VerRelType::AUTHENTICATION | VerRelType::ASSERTION),
			],
			nonce: 6 + 1,
		};
		...
		assert_noop!(
			DIDModule::add_keys(
				RuntimeOrigin::signed(alice),
				add_keys.clone(),
				DidSignature { did: Controller(did_1), key_id: 3u32.into(), sig }
			),
...
fn remove_keys_from_did() {
	ext().execute_with(|| {
		...
		for nonce in vec![1, 2, 4, 5, 6, 7, 8, 10, 10000] {
			let remove_keys =
				RemoveKeys { did: did_2, keys: vec![1u32.into()].into_iter().collect(), nonce };
		...
		let remove_keys =
			RemoveKeys { did: did_2, keys: vec![1u32.into()].into_iter().collect(), nonce: 5 + 1 };
		...
		let remove_keys =
			RemoveKeys { did: did_1, keys: vec![1u32.into()].into_iter().collect(), nonce: 2 + 1 };
		let sig = SigValue::ed25519(&remove_keys.to_state_change().encode(), &pair_ed_1);
		...
		let remove_keys =
			RemoveKeys { did: did_1, keys: vec![3u32.into()].into_iter().collect(), nonce: 3 + 1 };
		let sig = SigValue::ed25519(&remove_keys.to_state_change().encode(), &pair_ed_1);
		assert_ok!(DIDModule::remove_keys(
			RuntimeOrigin::signed(alice),
			remove_keys,
		...
		let remove_keys =
			RemoveKeys { did: did_2, keys: vec![1u32.into()].into_iter().collect(), nonce: 4 + 1 };
		let sig = SigValue::ed25519(&remove_keys.to_state_change().encode(), &pair_ed_1);
		assert_ok!(DIDModule::remove_keys(
			RuntimeOrigin::signed(alice),
			remove_keys,
		...
		assert_ok!(DIDModule::new_onchain(
			RuntimeOrigin::signed(alice),
			did_5,
			vec![UncheckedDidKey::new_with_all_relationships(PublicKey::sr25519(pk_sr_1))]
				.into_iter()
				.collect(),
			vec![did_1].into_iter().map(Controller).collect()
		));
		...
		let remove_keys =
			RemoveKeys { did: did_5, keys: vec![1u32.into()].into_iter().collect(), nonce: 5 + 1 };
		let sig = SigValue::ed25519(&remove_keys.to_state_change().encode(), &pair_ed_1);
		assert_ok!(DIDModule::remove_keys(
			RuntimeOrigin::signed(alice),
			remove_keys,
...
fn add_controllers_to_did() {
	ext().execute_with(|| {
		...
		let add_controllers =
			AddControllers { did: did_1, controllers: vec![].into_iter().collect(), nonce: 5 };
		...
		assert_ok!(DIDModule::add_controllers(
			RuntimeOrigin::signed(alice),
			add_controllers,
			DidSignature { did: Controller(did_3), key_id: 1u32.into(), sig }
		));
		assert!(!DIDModule::is_self_controlled(&did_2));
...
fn becoming_controller() {
	...
	ext().execute_with(|| {
		...
		let add_keys = AddKeys {
			did: did_2,
			keys: vec![UncheckedDidKey::new(PublicKey::ed25519(pk_ed), VerRelType::ASSERTION)],
			nonce: 5 + 1,
		};
		...
		let add_keys = AddKeys {
			did: did_2,
			keys: vec![UncheckedDidKey::new(pk_secp.clone(), VerRelType::CAPABILITY_INVOCATION)],
			nonce: 6 + 1,
		};
		let sig = SigValue::sr25519(&add_keys.to_state_change().encode(), &pair_sr);
		assert_ok!(DIDModule::add_keys(
...
fn any_controller_can_update() {
	...
	ext().execute_with(|| {
		...
		let add_keys = AddKeys {
			did: did_4,
			keys: vec![UncheckedDidKey::new(PublicKey::sr25519(pk_sr), VerRelType::NONE)],
			nonce: 3 + 1,
		};
	...
	ext().execute_with(|| {
		...
		assert_noop!(
			DIDModule::add_service_endpoint(
				...
				DidSignature { did: Controller(did.clone()), key_id: 1u32.into(), sig }
			),
			Error::<Test>::OnlyControllerCanUpdate
		);
		...
		assert_noop!(
			DIDModule::add_service_endpoint(
				RuntimeOrigin::signed(alice),
				add_service_endpoint.clone(),
				DidSignature { did: Controller(did.clone()), key_id: 2u32.into(), sig }
			),
			Error::<Test>::InsufficientVerificationRelationship
		);
		...
		for (id, ep) in vec![
			...
			(
				...
				ServiceEndpoint {
					types: ServiceEndpointType::LINKED_DOMAINS,
					origins: vec![vec![45; 55].into(), vec![].into()], /* All provided origins
					                                                    * mut be non-empty */
				},
			),
			(
			...
			let add_service_endpoint =
				AddServiceEndpoint { did: did.clone(), id: id.into(), endpoint: ep, nonce: 5 + 1 };
			let sig = SigValue::sr25519(&add_service_endpoint.to_state_change().encode(), &pair_sr);
		...
		let rem_service_endpoint =
			RemoveServiceEndpoint { did: did.clone(), id: endpoint_1_id.clone(), nonce: 7 + 1 };
		...
		let rem_service_endpoint =
			RemoveServiceEndpoint { did: did.clone(), id: vec![].into(), nonce: 7 + 1 };
		...
		let rem_service_endpoint =
			RemoveServiceEndpoint { did: did.clone(), id: endpoint_1_id.clone(), nonce: 8 + 1 };
		let sig = SigValue::sr25519(&rem_service_endpoint.to_state_change().encode(), &pair_sr);
		assert_noop!(
			DIDModule::remove_service_endpoint(
				RuntimeOrigin::signed(alice),
		...
		let rem_service_endpoint =
			RemoveServiceEndpoint { did: did.clone(), id: endpoint_2_id.clone(), nonce: 8 + 1 };
		let sig = SigValue::sr25519(&rem_service_endpoint.to_state_change().encode(), &pair_sr);
		...
		let rem_did = DidRemoval { did: did.clone(), nonce: 9 + 1 };
...
fn did_removal() {
	...
	ext().execute_with(|| {
		...
		assert_ok!(DIDModule::new_onchain(
			RuntimeOrigin::signed(alice),
			did_2,
			vec![UncheckedDidKey::new(PublicKey::ed25519(pk_ed), VerRelType::AUTHENTICATION)],
			vec![did_1].into_iter().map(Controller).collect()
		));
		assert!(!DIDModule::is_self_controlled(&did_2));
		check_did_detail(&did_2, 1, 0, 1, 10);
		...
		let rem_did = DidRemoval { did: did_2, nonce: 10 + 1 };
		let sig = SigValue::ed25519(&rem_did.to_state_change().encode(), &pair_ed);
		assert_noop!(
			DIDModule::remove_onchain_did(
				RuntimeOrigin::signed(alice),
		...
		let rem_did = DidRemoval { did: did_2, nonce: 5 + 1 };
		let sig = SigValue::sr25519(&rem_did.to_state_change().encode(), &pair_sr);
		assert_ok!(DIDModule::remove_onchain_did(
			RuntimeOrigin::signed(alice),
			rem_did.clone(),
		...
		let rem_did = DidRemoval { did: did_3, nonce: 5 };
		let sig = SigValue::sr25519(&rem_did.to_state_change().encode(), &pair_sr);
		assert_noop!(
			DIDModule::remove_onchain_did(
				RuntimeOrigin::signed(alice),
		...
		let rem_did = DidRemoval { did: did_3, nonce: 6 + 1 };
		let sig = SigValue::sr25519(&rem_did.to_state_change().encode(), &pair_sr);
		assert_ok!(DIDModule::remove_onchain_did(
			RuntimeOrigin::signed(alice),
			rem_did.clone(),
		...
		// longer remove did_4
		let rem_did = DidRemoval { did: did_4, nonce: 15 + 1 };
		let sig = SigValue::ed25519(&rem_did.to_state_change().encode(), &pair_ed);
		assert_noop!(
			DIDModule::remove_onchain_did(
				RuntimeOrigin::signed(alice),
		...
		let rem_did = DidRemoval { did: did_4, nonce: 20 + 1 };
		let sig = SigValue::sr25519(&rem_did.to_state_change().encode(), &pair_sr);
		assert_ok!(DIDModule::remove_onchain_did(
			RuntimeOrigin::signed(alice),
			rem_did.clone(),
			DidSignature { did: Controller(did_4), key_id: 1u32.into(), sig }
		));
		ensure_onchain_did_gone(&did_4);

		// did_1 removes itself
		let rem_did = DidRemoval { did: did_1, nonce: 7 + 1 };
		let sig = SigValue::sr25519(&rem_did.to_state_change().encode(), &pair_sr);
		assert_ok!(DIDModule::remove_onchain_did(
			RuntimeOrigin::signed(alice),
			rem_did.clone(),
...
fn batched_did_changes() {
	...
	ext().execute_with(|| {
		...
		let add_keys = AddKeys {
			did: did_1,
			keys: vec![UncheckedDidKey::new(PublicKey::x25519(pk_ed), VerRelType::KEY_AGREEMENT)],
			nonce: 10 + 1,
		};
		let sig = SigValue::sr25519(&add_keys.to_state_change().encode(), &pair_sr);
		...
		assert_ok!(DIDModule::add_service_endpoint(
			RuntimeOrigin::signed(alice),
			add_service_endpoint.clone(),
			DidSignature { did: Controller(did_1), key_id: 1u32.into(), sig: sig_1 }
		));
		...
		assert_ok!(DIDModule::add_keys(
			RuntimeOrigin::signed(alice),
			add_keys_2,
			DidSignature { did: Controller(did_1), key_id: 1u32.into(), sig: sig_2 }
		));
		assert_ok!(DIDModule::add_service_endpoint(
			RuntimeOrigin::signed(alice),
			add_service_endpoint.clone(),
			DidSignature { did: Controller(did_1), key_id: 1u32.into(), sig: sig_3 }
		));
...
fn valid_ver_rels_encoding_decoding() {
	assert_err!(VerRelType::decode(&mut &999u32.encode()[..]), codec::Error::from("Invalid value"));
	assert_ok!(VerRelType::decode(&mut &7u32.encode()[..]), VerRelType::ALL_FOR_SIGNING);
}
