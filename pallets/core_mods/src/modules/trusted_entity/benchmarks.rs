...
oneof(&[did]), add_only: false}}).unwrap();
        ...
        crate::trusted_entity::Pallet::<T>::add_trusted_entity_(
            AddTrustedEntityRaw {
                /// The authorizer on which to operate
               authorizer_id: authorizer_id,
               /// entity ids which will be added
        ...
        let remove_trusted_entity_raw = RemoveTrustedEntityRaw {
            /// The authorizer on which to operate
           authorizer_id: authorizer_id,
           /// entity ids which will be added
           entity_ids: entity_ids.clone(),
        ...
        let remove_trusted_entity = RemoveTrustedEntity::new_with_nonce(remove_trusted_entity_raw.clone(), 1u32.into());
        let sig = pair.sign(&remove_trusted_entity.to_state_change().encode());
        let signature = DidSignature::new(did, 1u32, sig);

    }: remove_trusted_entity(RawOrigin::Signed(caller), remove_trusted_entity_raw, vec![DidSigs { sig: signature, nonce:
1u32.into() }]) 	verify {
        assert!(entity_ids
            .iter()
            .all(|id| !TrustedEntities::contains_key(authorizer_id, id)));
