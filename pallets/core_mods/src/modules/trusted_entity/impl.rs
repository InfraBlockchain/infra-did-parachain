...
impl<T: Config + Debug> Module<T> {
	pub(super) fn new_authorizer_(
		...
		// check
		ensure!(new_authorizer.policy.valid(), TrustError::<T>::InvalidPolicy);
		ensure!(!Authorizers::contains_key(&id), TrustError::<T>::AuthorizerExists);
		ensure!(
		...
		E: From<TrustError<T>> + From<did::Error<T>> + From<NonceError>,
	{
		...
		Authorizers::try_mutate_exists(action.target(), |authorizer_opt| {
			...
			for DidSigs { sig, nonce } in proof {
				...
				did_detail.try_update(nonce).map_err(|_| TrustError::<T>::IncorrectNonce)?;
