...
mod errors {
    ...
    fn notauthorized() {
        ...
        fn assert_add_trusted_entity_err(
            policy: Policy,
            signers: &[(Did, &sr25519::Pair)],
        ) -> DispatchError {
            ...
            let add_trusted_entity = AddTrustedEntityRaw {
                _marker: PhantomData,
                authorizer_id,
                entity_ids: random::<[TrustedEntityId; 32]>().iter().cloned().collect(),
            };
            let pauth = get_pauth(&add_trusted_entity, signers);
            dbg!(&add_trusted_entity);
            dbg!(&pauth);
            TrustedEntityMod::add_trusted_entity(
        ...
        for (pol, set, description) in cases {
            dbg!(description);
            assert_eq!(
                assert_add_trusted_entity_err(pol.clone(), set),
    ...
    fn notauthorized_wrong_command() {
        ...
        let remove_trusted_entity = RemoveTrustedEntityRaw {
            _marker: PhantomData,
            authorizer_id,
            entity_ids: once(Default::default()).collect(),
        };
        let ur_proof = get_pauth(&remove_trusted_entity, &[(DIDA, &kpa)]);
        TrustedEntityMod::remove_trusted_entity(
            RuntimeOrigin::signed(ABBA),
            remove_trusted_entity.clone(),
            ur_proof,
        )
        .unwrap();
    ...
    fn emtpy_payload() {
        ...
        TrustedEntityMod::new_authorizer(RuntimeOrigin::signed(ABBA), ar.clone()).unwrap();
        let add_trusted_entity_raw = AddTrustedEntityRaw {
            _marker: PhantomData,
            authorizer_id,
            entity_ids: Default::default(),
        };
        let proof = get_pauth(&add_trusted_entity_raw, &[(DIDA, &kpa)]);

        assert_noop!(
            TrustedEntityMod::add_trusted_entity(
                RuntimeOrigin::signed(ABBA),
                add_trusted_entity_raw,
                proof
            ),
            err
        );
    ...
    fn incorrect_nonce() {
        ...
        let proof = get_pauth(&add_trusted_entity, &[(DIDA, &kpa)]);
        ...
        let proof = get_pauth(&remove_trusted_entity, &[(DIDA, &kpa)]);
...
mod calls {
    ...
    // `Call` in `test_common`
    use super::super::{Authorizers, Call as TrstCall, TrustedEntities};
    use alloc::collections::BTreeSet;
    ...
    #[test]
    fn add_trusted_entity() {
        if !in_ext() {
            return ext().execute_with(add_trusted_entity);
        }
    ...
    #[test]
    fn remove_trusted_entity() {
        if !in_ext() {
            return ext().execute_with(remove_trusted_entity);
        }
        ...
        for (i, (action, ids, line_no)) in cases.into_iter().enumerate() {
            ...
            match action {
                ...
                Action::RemoveAuthorizer => {
                    assert!(!entity_ids
                        .iter()
                        .any(|id| TrustedEntities::contains_key(authorizer_id, id)));
                }
            }
    ...
    fn _all_included(dummy: TrstCall<Test>) {
        match dummy {
            ...
            | TrstCall::add_trusted_entity { .. }
            | TrstCall::remove_trusted_entity { .. }
...
mod test {
    ...
    fn ensure_auth() {
        ...
        for (i, (line_no, policy, signers, expect_success)) in cases.into_iter().enumerate() {
            ...
            let command = &add_trusted_entity;
    ...
    #[test]
    /// Exercises the trusted entity status convenience getter, get_trusted_entity.
    fn get_trusted_entity() {
        if !in_ext() {
            return ext().execute_with(get_trusted_entity);
        }
        ...
        assert_eq!(
            TrustedEntityMod::get_trusted_entity(authorizer_id, revid),
            None
        );
        TrustedEntityMod::add_trusted_entity(
            RuntimeOrigin::signed(ABBA),
