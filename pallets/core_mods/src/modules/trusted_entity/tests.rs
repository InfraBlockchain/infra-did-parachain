...
mod errors {
	...
	fn invalidpolicy() {
		...
		let ar = AddAuthorizer {
			id: RGA,
			new_authorizer: Authorizer { policy: trusted_entity_oneof(&[]), add_only: false },
		};
	...
	fn notauthorized() {
		...
		let cases: &[(Policy, &[(Did, &sr25519::Pair)], &str)] = &[
			...
			(trusted_entity_oneof(&[a]), &[(b, &kpb)], "wrong account; wrong key"),
			(trusted_entity_oneof(&[a]), &[(a, &kpb)], "correct account; wrong key"),
			(trusted_entity_oneof(&[a]), &[(a, &kpb)], "wrong account; correct key"),
			(trusted_entity_oneof(&[a, b]), &[(c, &kpc)], "account not a controller"),
			(trusted_entity_oneof(&[a, b]), &[(a, &kpa), (b, &kpb)], "two signers"),
			(trusted_entity_oneof(&[a]), &[], "one controller; no sigs"),
			(trusted_entity_oneof(&[a, b]), &[], "two controllers; no sigs"),
		];
	...
	fn AuthorizerExists() {
		...
		let authorizer = Authorizer { policy: trusted_entity_oneof(&[DIDA]), add_only: false };
		let ar = AddAuthorizer { id: RGA, new_authorizer: authorizer };
	...
	fn incorrect_nonce() {
		...
		let ar = AddAuthorizer {
			id: authorizer_id,
			new_authorizer: Authorizer { policy: trusted_entity_oneof(&[DIDA]), add_only: false },
		};
	...
	fn addonly() {
		...
		let ar = AddAuthorizer {
			id: authorizer_id,
			new_authorizer: Authorizer { policy: trusted_entity_oneof(&[DIDA]), add_only: true },
		};
...
mod calls {
	...
	fn add_issuer() {
		...
		for (i, ids) in cases.into_iter().enumerate() {
			...
			assert!(ids.iter().all(|id| Issuers::contains_key(authorizer_id, id)));
	...
	fn remove_issuer() {
		...
		let cases: &[(Action, &[TrustedEntityId], u32)] = &[
			...
			(Action::RemoveTrustedEntity, &[random(), random(), random()], line!()),
		...
		for (i, (action, ids, line_no)) in cases.into_iter().enumerate() {
			...
			match action {
				...
				Action::AddAuthorizer => {
					assert!(entity_ids.iter().all(|id| Issuers::contains_key(authorizer_id, id)));
				},
				Action::RemoveAuthorizer => {
					assert!(!entity_ids.iter().any(|id| Issuers::contains_key(authorizer_id, id)));
				},
			}
...
mod test {
	...
	fn ensure_auth() {
		...
		let cases: &[(u32, Policy, &[(Did, &sr25519::Pair)], bool)] = &[
			...
			(line!(), trusted_entity_oneof(&[a, b]), &[(a, &kpa), (b, &kpb)], false), // two signers
		...
		for (i, (line_no, policy, signers, expect_success)) in cases.into_iter().enumerate() {
			...
			Authorizers::insert(RGA, Authorizer { policy: policy.clone(), add_only: false });
	...
	fn get_issuer() {
		...
		TrustedEntityMod::new_authorizer(RuntimeOrigin::signed(ABBA), ar).unwrap();
		let add_issuer =
			AddIssuerRaw { _marker: PhantomData, authorizer_id, entity_ids: once(revid).collect() };
		let proof = get_pauth(&add_issuer, &[(DIDA, &kpa)]);
