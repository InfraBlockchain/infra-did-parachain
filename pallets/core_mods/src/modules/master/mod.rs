 ...
 * are submitted in a single transaction. If enough valid votes endorse the proposal, the
 * proposal is run as root. If the running the proposal as root succeeds, a new round of voting
 * is started.
 *
...
impl Default for Membership {
	fn default() -> Self {
		Membership { members: BTreeSet::new(), vote_requirement: 1 }
	}
}
...
// Minimum weight of Master's extrinsics. This is not based on any computation but only there to
// account for some in-memory operations
const MIN_WEIGHT: Weight = Weight::from_ref_time(10_000);

/// Minimum weight for master's extrinsics. Considers cost of signature verification and update to
/// round no
...
decl_event! {
		...
		<T as Config>::RuntimeCall
	{
		...
		Executed(Vec<Did>, Box<RuntimeCall>),
		/// The membership of Master has changed.
		UnderNewOwnership,
		/// A proposal failed to execute
		ExecutionFailed(Vec<Did>, Box<RuntimeCall>, DispatchError),
	}
}
...
impl<T: Config + Debug> Module<T> {
	/// Execute a call as Root origin after verifying signatures in `auth`. If `given_weight` is
	/// None, then it computes the weight by considering the cost of signature verification and cost
	/// of executing the call. If `given_weight` has a value then that is considered as weight.
	...
	/// However, since this call is made by entities that not adversarial, the behavior is not
	...
	fn execute_(
		...
		for a in auth.iter() {
			...
			did_detail.try_update(nonce).map_err(|_| MasterError::<T>::IncorrectNonce)?;
			// Verify signature
		...
		let dispatch_result =
			proposal.clone().dispatch_bypass_filter(system::RawOrigin::Root.into());
		...
		// Weight from dispatch's declaration. If dispatch does not return a weight in
		// `PostDispatchInfo`, then this weight is used.
		...
		// `post_info` does not have weight, use weight from declaration. Also add minimum weight
		// for execution
		let actual_weight = move |post_info: PostDispatchInfo| {
			given_weight.or_else(|| {
				Some(
					post_info.actual_weight.unwrap_or_else(|| dispatch_decl_weight) +
	...
	fn set_members_(membership: Membership) -> DispatchResult {
		// check
		ensure!(membership.vote_requirement != 0, MasterError::<T>::ZeroVoteRequirement);
		ensure!(
