...
fn execute_set_members() {
	ext().execute_with(|| {
		Members::set(Membership { members: set(&[]), vote_requirement: 0 });
		let new_members = Membership { members: set(&[newdid().0]), vote_requirement: 1 };
		let call =
			RuntimeCall::MasterMod(MasterCall::set_members { membership: new_members.clone() });
...
fn test_events() {
	ext().execute_with(|| {
		MasterMod::set_members(
			...
			Membership { members: set(&[newdid().0]), vote_requirement: 1 },
		)
		.unwrap();
	...
	ext().execute_with(|| {
		...
		Members::set(Membership { members: set(&[]), vote_requirement: 0 });
		MasterMod::execute(RuntimeOrigin::signed(0), Box::new(call.clone()), vec![]).unwrap();
		assert_eq!(master_events(), vec![Event::<Test>::Executed(vec![], Box::new(call))]);
	});
	...
	ext().execute_with(|| {
		...
		Members::set(Membership { members: set(&[dida, didb, didc]), vote_requirement: 2 });

		run_to_block(15);

		let call = RuntimeCall::System(system::Call::<Test>::set_storage { items: vec![] });
		let sc =
			MasterVoteRaw { _marker: PhantomData, proposal: call.encode(), round_no: Round::get() };
		...
		assert_eq!(
			master_events(),
			vec![Event::<Test>::Executed(sorted(vec![dida, didc]), Box::new(call))]
		);
	});
	...
	ext().execute_with(|| {
		...
		let call = RuntimeCall::MasterMod(MasterCall::set_members {
			membership: Membership { members: set(&[newdid().0]), vote_requirement: 1 },
		});
		MasterMod::execute(RuntimeOrigin::signed(0), Box::new(call.clone()), vec![]).unwrap();
		assert_eq!(
			master_events(),
			vec![Event::<Test>::UnderNewOwnership, Event::<Test>::Executed(vec![], Box::new(call)),]
		);
	});
...
fn no_members() {
	ext().execute_with(|| {
		...
		let sc =
			MasterVoteRaw { _marker: PhantomData, proposal: call.encode(), round_no: Round::get() };
		Members::set(Membership { members: set(&[]), vote_requirement: 1 });
...
fn valid_call() {
	ext().execute_with(|| {
		...
		let call =
			RuntimeCall::System(system::Call::<Test>::set_storage { items: vec![kv.clone()] });
		let sc =
			MasterVoteRaw { _marker: PhantomData, proposal: call.encode(), round_no: Round::get() };
		Members::set(Membership { members: set(&[dida, didb, didc]), vote_requirement: 2 });
...
fn all_members_vote() {
	ext().execute_with(|| {
		...
		let sc =
			MasterVoteRaw { _marker: PhantomData, proposal: call.encode(), round_no: Round::get() };
		Members::set(Membership { members: set(&[dida, didb, didc]), vote_requirement: 3 });
...
fn two_successful_rounds_of_voting() {
	ext().execute_with(|| {
...

		{
			...
			let sc = MasterVoteRaw { _marker: PhantomData, proposal: call.encode(), round_no: 0 };
...

		{
			...
			let sc = MasterVoteRaw { _marker: PhantomData, proposal: call.encode(), round_no: 1 };
...
fn err_bad_sig() {
	ext().execute_with(|| {
		...
		let (didb, _didbk) = newdid();
		let (didc, didck) = newdid();
		Members::set(Membership { members: set(&[dida, didb]), vote_requirement: 1 });
		let call =
			Box::new(RuntimeCall::System(system::Call::<Test>::set_storage { items: vec![] }));
		let sc = MasterVoteRaw { _marker: PhantomData, proposal: call.encode(), round_no: 0 };
		...
		{
			// wrong payload
			let sc = crate::revoke::RemoveRegistryRaw { registry_id: RGA, _marker: PhantomData };
			let pauth = get_pauth(&sc, &[(dida, &didak)]);
			let err =
				MasterMod::execute(RuntimeOrigin::signed(0), call.clone(), pauth).unwrap_err();
			assert_eq!(err, MasterError::<Test>::BadSig.into());
...
fn err_not_member() {
	ext().execute_with(|| {
		...
		let (didc, didck) = newdid();
		Members::set(Membership { members: set(&[dida]), vote_requirement: 1 });
		let call =
			Box::new(RuntimeCall::System(system::Call::<Test>::set_storage { items: vec![] }));
		let sc = MasterVoteRaw { _marker: PhantomData, proposal: call.encode(), round_no: 0 };
		let pauth = get_pauth(&sc, &[(didc, &didck)]);
...
fn err_insufficient_votes() {
	ext().execute_with(|| {
		...
		let sc =
			MasterVoteRaw { _marker: PhantomData, proposal: call.encode(), round_no: Round::get() };
		Members::set(Membership { members: set(&[dida, didb]), vote_requirement: 2 });
...
fn err_zero_vote_requirement() {
	ext().execute_with(|| {
		for m in [
			Membership { members: set(&[]), vote_requirement: 0 },
			Membership { members: set(&[newdid().0]), vote_requirement: 0 },
		]
		.iter()
		.cloned()
		{
...
fn err_vote_requirement_to_high() {
	ext().execute_with(|| {
		for m in [
			Membership { members: set(&[]), vote_requirement: 1 },
			Membership { members: set(&[newdid().0]), vote_requirement: 2 },
			Membership { members: set(&[newdid().0]), vote_requirement: 3 },
			Membership { members: set(&[newdid().0]), vote_requirement: u64::MAX },
		]
		.iter()
		.cloned()
		{
...
fn master_events() -> Vec<Event<Test>> {
	system::Pallet::<Test>::events()
		...
		.filter_map(|event_record| {
			let system::EventRecord::<TestEvent, H256> { phase, event, topics: _ } = event_record;
