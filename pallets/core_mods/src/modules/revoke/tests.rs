...
mod errors {
	...
	fn invalidpolicy() {
		...
		let ar =
			AddRegistry { id: RGA, new_registry: Registry { policy: oneof(&[]), add_only: false } };
	...
	fn regexists() {
		...
		let reg = Registry { policy: oneof(&[DIDA]), add_only: false };
		let ar = AddRegistry { id: RGA, new_registry: reg };
		RevoMod::new_registry(RuntimeOrigin::signed(ABBA), ar.clone()).unwrap();
		let err = RevoMod::new_registry(RuntimeOrigin::signed(ABBA), ar.clone()).unwrap_err();
	...
	fn incorrect_nonce() {
		...
		let ar = AddRegistry {
			id: registry_id,
			new_registry: Registry { policy: oneof(&[DIDA]), add_only: false },
		};
	...
	fn addonly() {
		...
		let ar = AddRegistry {
			id: registry_id,
			new_registry: Registry { policy: oneof(&[DIDA]), add_only: true },
		};
	...
	fn unrevoke() {
		...
		for (i, (action, ids, line_no)) in cases.into_iter().enumerate() {
			...
			match action {
				...
				Action::AsrtRv => {
					assert!(revoke_ids.iter().all(|id| Revocations::contains_key(registry_id, id)));
				},
				Action::AsrtNR => {
					assert!(!revoke_ids
						.iter()
...
mod test {
	...
	fn ensure_auth() {
		...
		for (i, (line_no, policy, signers, expect_success)) in cases.into_iter().enumerate() {
			...
			Registries::insert(RGA, Registry { policy: policy.clone(), add_only: false });
