...
fn invalid_sig_a() {
	ext().execute_with(|| {
		...
		let (dida, kpa) = newdid();
		let mut att = Attestation { priority: 1, iri: None };
		...
		let err = AttestMod::set_claim(
			RuntimeOrigin::signed(0),
			SetAttestationClaim { attest: att.clone(), nonce: 10 + 2 },
			sig,
		)
		.unwrap_err();
...
fn priority_face_off() {
	ext().execute_with(|| {
		...
		// same iri
		set_claim(&did, &Attestation { priority: 1, iri: None }, &kp, 10 + 1).unwrap();
		check_nonce(&did, 10 + 1);
		assert_eq!(
			set_claim(&did, &Attestation { priority: 1, iri: None }, &kp, 11 + 1).unwrap_err(),
			Er::PriorityTooLow.into()
		);

		// different iris
		set_claim(&did, &Attestation { priority: 2, iri: Some(vec![0]) }, &kp, 11 + 1).unwrap();
		check_nonce(&did, 11 + 1);

		assert_eq!(
			set_claim(&did, &Attestation { priority: 2, iri: Some(vec![0, 2, 3]) }, &kp, 12 + 1)
				.unwrap_err(),
			Er::PriorityTooLow.into()
		);
	});
...
fn priority_battle_royale() {
	ext().execute_with(|| {
		...
		for priority in &prios {
			...
			let _ = set_claim(&did, &Attestation { priority: *priority, iri: None }, &kp, nonce)
				.and_then(|_| {
		...
		assert_eq!(Attestations::get(did).priority, prios.iter().max().unwrap().clone());
	});
}
...
fn max_priority_is_final() {
	ext().execute_with(|| {
		...
		set_claim(&did, &Attestation { priority: u64::max_value(), iri: None }, &kp, 10 + 1)
			.unwrap();
		check_nonce(&did, 10 + 1);
		let err =
			set_claim(&did, &Attestation { priority: u64::max_value(), iri: None }, &kp, 11 + 1)
				.unwrap_err();
		assert_eq!(err, Er::PriorityTooLow.into());
	});
}
...
fn set_some_attestation() {
	ext().execute_with(|| {
		...
		let (did, kp) = newdid();
		let did = Attester(did);
		assert_eq!(Attestations::get(did), Attestation { priority: 0, iri: None });
		check_nonce(&did, 10);
		set_claim(&did, &Attestation { priority: 1, iri: Some(vec![0, 1, 2]) }, &kp, 10 + 1)
			.unwrap();
		check_nonce(&did, 10 + 1);
		assert_eq!(Attestations::get(did), Attestation { priority: 1, iri: Some(vec![0, 1, 2]) });
	});
}
...
fn skip_prio() {
	ext().execute_with(|| {
		...
		for (i, priority) in [1, 2, 4].iter().enumerate() {
			...
			check_nonce(&did, nonce - 1);
			set_claim(&did, &Attestation { priority: *priority, iri: None }, &kp, nonce).unwrap();
			check_nonce(&did, nonce);
		}
	});
}
