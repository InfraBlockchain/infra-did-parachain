...
fn bbs_plus_events() -> Vec<super::Event> {
	System::events()
		...
		.filter_map(|event_record| {
			let system::EventRecord::<TestEvent, H256> { phase: _p, event, topics: _t } =
				event_record;
			match event {
...
fn add_remove_params() {
	ext().execute_with(|| {
		...
		let ap = AddBBSPlusParams { params: params.clone(), nonce: next_nonce };
		let sig = sign_add_params::<Test>(&author_kp, &ap, author.clone(), 1);

		assert_eq!(ParamsCounter::get(&BBSPlusParamsOwner(author)), IncId::from(0u8));
		assert_err!(
			BBSPlusMod::add_params(
				RuntimeOrigin::signed(1),
				AddBBSPlusParams { params: params.clone(), nonce: next_nonce },
				sig.clone()
			),
		...
		assert!(!bbs_plus_events()
			.contains(&super::Event::ParamsAdded(BBSPlusParamsOwner(author), 1u8.into())));
		check_nonce(&author, next_nonce - 1);
		...
		assert_eq!(ParamsCounter::get(&BBSPlusParamsOwner(author)), IncId::from(0u8));
		assert_eq!(BbsPlusParams::get(&BBSPlusParamsOwner(author), IncId::from(1u8)), None);
		assert!(!bbs_plus_events()
			.contains(&super::Event::ParamsAdded(BBSPlusParamsOwner(author), 1u8.into())));
		check_nonce(&author, next_nonce - 1);
		...
		next_nonce += 1;
		assert_eq!(ParamsCounter::get(&BBSPlusParamsOwner(author)), IncId::from(1u8));
		assert_eq!(
		...
		assert_eq!(BbsPlusParams::get(&BBSPlusParamsOwner(author), IncId::from(2u8)), None);
		let params_1 = BBSPlusParameters {
			label: None,
			curve_type: CurveType::Bls12381,
			bytes: vec![1u8; 100].into(),
		};
		let ap = AddBBSPlusParams { params: params_1.clone(), nonce: next_nonce };
		let sig = sign_add_params::<Test>(&author_kp, &ap, author.clone(), 1);
		BBSPlusMod::add_params(
			RuntimeOrigin::signed(1),
			AddBBSPlusParams { params: params_1.clone(), nonce: next_nonce },
			sig,
		)
		.unwrap();
		check_nonce(&author, next_nonce);
		next_nonce += 1;
		assert_eq!(ParamsCounter::get(&BBSPlusParamsOwner(author)), IncId::from(2u8));
		assert_eq!(
			BbsPlusParams::get(&BBSPlusParamsOwner(author), IncId::from(2u8)),
			Some(params_1)
		);
		assert!(bbs_plus_events()
			.contains(&super::Event::ParamsAdded(BBSPlusParamsOwner(author), 2u8.into())));
		...
		let ap = AddBBSPlusParams { params: params_2.clone(), nonce: next_nonce_1 };
		let sig = sign_add_params::<Test>(&author_1_kp, &ap, author_1.clone(), 1);
		assert_eq!(ParamsCounter::get(&BBSPlusParamsOwner(author_1)), IncId::from(0u8));
		assert_eq!(BbsPlusParams::get(&BBSPlusParamsOwner(author_1), IncId::from(1u8)), None);
		BBSPlusMod::add_params(
			RuntimeOrigin::signed(1),
			AddBBSPlusParams { params: params_2.clone(), nonce: next_nonce_1 },
			sig,
		)
		.unwrap();
		check_nonce(&author_1, next_nonce_1);
		next_nonce_1 += 1;
		assert_eq!(ParamsCounter::get(&BBSPlusParamsOwner(author_1)), IncId::from(1u8));
		assert_eq!(
		...
		assert_eq!(ParamsCounter::get(&BBSPlusParamsOwner(author)), IncId::from(2u8));
		assert!(bbs_plus_events()
			.contains(&super::Event::ParamsAdded(BBSPlusParamsOwner(author_1), 1u8.into())));

		run_to_block(30);

		assert_eq!(BbsPlusParams::get(&BBSPlusParamsOwner(author), IncId::from(3u8)), None);
		let params_3 = BBSPlusParameters {
			label: None,
			curve_type: CurveType::Bls12381,
			bytes: vec![8u8; 100].into(),
		};
		let ap = AddBBSPlusParams { params: params_3.clone(), nonce: next_nonce };
		let sig = sign_add_params::<Test>(&author_kp, &ap, author.clone(), 1);
		BBSPlusMod::add_params(
			RuntimeOrigin::signed(1),
			AddBBSPlusParams { params: params_3.clone(), nonce: next_nonce },
			sig,
		)
		.unwrap();
		check_nonce(&author, next_nonce);
		next_nonce += 1;
		assert_eq!(ParamsCounter::get(&BBSPlusParamsOwner(author)), IncId::from(3u8));
		assert_eq!(
			BbsPlusParams::get(&BBSPlusParamsOwner(author), IncId::from(3u8)),
			Some(params_3.clone())
		);
		assert!(bbs_plus_events()
			.contains(&super::Event::ParamsAdded(BBSPlusParamsOwner(author), 3u8.into())));

		let rf = (BBSPlusParamsOwner(author.clone()), 5u8.into());
		let rp = RemoveBBSPlusParams { params_ref: rf, nonce: next_nonce };
		...
		let mut rp = RemoveBBSPlusParams { params_ref: rf, nonce: next_nonce_1 };
		...
		assert!(bbs_plus_events()
			.contains(&super::Event::ParamsRemoved(BBSPlusParamsOwner(author), 2u8.into())));

		let rp = RemoveBBSPlusParams::<Test> { params_ref: rf, nonce: next_nonce };
		let sig = sign_remove_params(&author_kp, &rp, author.clone(), 1);
		// Cannot remove as already removed
		assert_err!(
			BBSPlusMod::remove_params(
				RuntimeOrigin::signed(1),
				RemoveBBSPlusParams { params_ref: rf, nonce: next_nonce },
				sig.clone()
			),
			Error::<Test>::ParamsDontExist
		);
		...
		let rp = RemoveBBSPlusParams { params_ref: rf, nonce: next_nonce_1 };
		let sig = sign_remove_params(&author_1_kp, &rp, author_1.clone(), 1);
		BBSPlusMod::remove_params(RuntimeOrigin::signed(1), rp, sig.clone()).unwrap();
		check_nonce(&author_1, next_nonce_1);
		next_nonce_1 += 1;
		...
		assert!(bbs_plus_events()
			.contains(&super::Event::ParamsRemoved(BBSPlusParamsOwner(author_1), 1u8.into())));

		let rp = RemoveBBSPlusParams::<Test> { params_ref: rf, nonce: next_nonce_1 };
		let sig = sign_remove_params(&author_1_kp, &rp, author_1.clone(), 1);
		// Cannot remove as already removed
		assert_err!(
			BBSPlusMod::remove_params(
				RuntimeOrigin::signed(1),
				RemoveBBSPlusParams { params_ref: rf, nonce: next_nonce_1 },
				sig.clone()
			),
			Error::<Test>::ParamsDontExist
		);
		...
		assert!(bbs_plus_events()
			.contains(&super::Event::ParamsRemoved(BBSPlusParamsOwner(author), 3u8.into())));
		...
		assert_eq!(BbsPlusParams::get(&BBSPlusParamsOwner(author), IncId::from(1u8)), None);
		assert!(bbs_plus_events()
			.contains(&super::Event::ParamsRemoved(BBSPlusParamsOwner(author), 1u8.into())));
	});
}
...
fn add_remove_public_key() {
	ext().execute_with(|| {
		...
		let ak = AddBBSPlusPublicKey { key: key.clone(), did: author.clone(), nonce: next_nonce };
		...
		assert_eq!(ParamsCounter::get(&BBSPlusParamsOwner(author)), IncId::from(0u8));
		assert_eq!(BbsPlusKeys::get(&author, IncId::from(1u8)), None);
		assert_eq!(BbsPlusKeys::get(&author, IncId::from(2u8)), Some(key.clone()));
		assert!(bbs_plus_events().contains(&super::Event::KeyAdded(author, 2u8.into())));
		...
		let ak =
			AddBBSPlusPublicKey { key: key_2.clone(), did: author_1.clone(), nonce: next_nonce_1 };
		...
		next_nonce_1 += 1;
		assert_eq!(ParamsCounter::get(&BBSPlusParamsOwner(author_1)), IncId::from(0u8));
		assert_eq!(BbsPlusKeys::get(&author_1, IncId::from(2u8)), Some(key_2.clone()));
		assert_eq!(ParamsCounter::get(&BBSPlusParamsOwner(author)), IncId::from(0u8));
		...
		let ak = AddBBSPlusPublicKey { key: key_3.clone(), did: author.clone(), nonce: next_nonce };
		let sig = sign_add_key(&author_kp, &ak, author.clone(), 1);
		BBSPlusMod::add_public_key(RuntimeOrigin::signed(1), ak, sig).unwrap();
		check_nonce(&author, next_nonce);
		next_nonce += 1;
		assert_eq!(ParamsCounter::get(&BBSPlusParamsOwner(author)), IncId::from(0u8));
		assert_eq!(BbsPlusKeys::get(&author, IncId::from(4u8)), Some(key_3.clone()));
		assert!(bbs_plus_events().contains(&super::Event::KeyAdded(author, 3u8.into())));

		run_to_block(60);

		let rf = (author.clone(), 5u8.into());
		let rk = RemoveBBSPlusPublicKey { key_ref: rf, did: author.clone(), nonce: next_nonce };
		...
		let rk = RemoveBBSPlusPublicKey { key_ref: rf, did: author_1.clone(), nonce: next_nonce_1 };
		...
		let ak =
			AddBBSPlusPublicKey { key: key_4.clone(), did: author_1.clone(), nonce: next_nonce_1 };
		let sig = sign_add_key(&author_kp_1, &ak, author_1.clone(), 1);
		assert_err!(
			BBSPlusMod::add_public_key(RuntimeOrigin::signed(1), ak, sig.clone()),
			Error::<Test>::ParamsDontExist
		...
		check_nonce(&author_1, next_nonce_1);
		assert_eq!(ParamsCounter::get(&BBSPlusParamsOwner(author_1)), IncId::from(0u8));
		assert_eq!(BbsPlusKeys::get(&author_1, IncId::from(3u8)), Some(key_4.clone()));
		assert!(bbs_plus_events().contains(&super::Event::KeyAdded(author_1, 3u8.into())));

		let ak = AddBBSPlusPublicKey { key: key_4.clone(), did: author.clone(), nonce: next_nonce };
		let sig = sign_add_key(&author_kp, &ak, author.clone(), 1);
		BBSPlusMod::add_public_key(RuntimeOrigin::signed(1), ak, sig.clone()).unwrap();
		check_nonce(&author, next_nonce);
		assert_eq!(ParamsCounter::get(&BBSPlusParamsOwner(author)), IncId::from(1u8));
...
fn add_remove_public_key_by_controller() {
	ext().execute_with(|| {
		...
		let ak = AddBBSPlusPublicKey { key: key.clone(), did: did_1, nonce: next_nonce };
		...
		assert_eq!(BbsPlusKeys::get(&did_1, IncId::from(2u8)), Some(key.clone()));
		...
		let rk = RemoveBBSPlusPublicKey { key_ref: rf, did: did_1, nonce: next_nonce };
...
fn add_params_keys() {
	ext().execute_with(|| {
		...
		assert_eq!(ParamsCounter::get(&BBSPlusParamsOwner(author)), IncId::from(0u8));
		assert_eq!(ParamsCounter::get(&BBSPlusParamsOwner(author_1)), IncId::from(0u8));
		assert_eq!(ParamsCounter::get(&BBSPlusParamsOwner(author_2)), IncId::from(0u8));
		...
		.is_ok());
		assert_eq!(BbsPlusKeys::get(&author, IncId::from(2u8)), Some(key.clone()));
		assert_eq!(BbsPlusKeys::get(&author, IncId::from(3u8)), Some(key_1.clone()));
		...
		assert_eq!(BbsPlusKeys::get(&author, IncId::from(2u8)), Some(key.clone()));
		assert_eq!(BbsPlusKeys::get(&author, IncId::from(3u8)), Some(key_1.clone()));
		assert_eq!(BbsPlusKeys::get(&author, IncId::from(4u8)), Some(key_2.clone()));

		run_to_block(70);

		let did_detail = DIDModule::onchain_did_details(&author).unwrap();
		assert!(BBSPlusMod::add_params_(
			AddBBSPlusParams { params: params_1.clone(), nonce: did_detail.next_nonce().unwrap() },
			BBSPlusParamsOwner(author)
		)
		.is_ok());
		assert_eq!(ParamsCounter::get(&BBSPlusParamsOwner(author)), IncId::from(2u8));
		...
		.is_ok());
		assert_eq!(BbsPlusKeys::get(&author_1, IncId::from(2u8)), Some(key.clone()));

		run_to_block(90);

		let did_detail_1 = DIDModule::onchain_did_details(&author_1).unwrap();
		assert!(BBSPlusMod::add_params_(
			AddBBSPlusParams { params: params.clone(), nonce: did_detail_1.next_nonce().unwrap() },
		...
		.is_ok());
		assert_eq!(BbsPlusKeys::get(&author_1, IncId::from(2u8)), Some(key.clone()));
		assert_eq!(BbsPlusKeys::get(&author_1, IncId::from(3u8)), Some(key_1.clone()));
	});
}
...
fn get_params_and_keys() {
	ext().execute_with(|| {
		...
		assert_eq!(BBSPlusMod::get_params_by_did(&BBSPlusParamsOwner(author)).len(), 0);
		assert_eq!(BBSPlusMod::get_params_by_did(&BBSPlusParamsOwner(author_1)).len(), 0);
		assert_eq!(BBSPlusMod::get_public_key_with_params(&(author, 0u8.into())), None);
		assert_eq!(BBSPlusMod::get_public_key_with_params(&(author_1, 0u8.into())), None);
		...
		assert_eq!(BBSPlusMod::get_params_by_did(&BBSPlusParamsOwner(author)), {
			let mut m = BTreeMap::new();
			m.insert(1u8.into(), params.clone());
			m
		});

		assert_eq!(BBSPlusMod::get_params_by_did(&BBSPlusParamsOwner(author_1)), {
			let mut m = BTreeMap::new();
