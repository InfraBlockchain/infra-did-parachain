...
fn create_blob(
	...
	let bl = Blob { id, blob: content.clone().into() };
	println!("did: {:?}", author);
...
fn add_blob() {
	fn add(size: usize, block_no: u64) {
		...
		create_blob(id, noise.clone(), BlobOwner(author), author_kp, block_no + 1).unwrap();
...
fn err_blob_already_exists() {
	ext().execute_with(|| {
		...
		create_blob(id, random_bytes(10), BlobOwner(author), author_kp.clone(), 10 + 1).unwrap();
	...
	ext().execute_with(|| {
		{
			...
			let (author, author_kp) = newdid();
			let bl = Blob { id: rand::random(), blob: random_bytes(10).into() };
			let att = crate::attest::SetAttestationClaim::<Test> {
				attest: crate::attest::Attestation { priority: 1, iri: None },
				nonce: 10 + 1,
			};
			check_nonce(&author, 10);
			let err = BlobMod::new(
				RuntimeOrigin::signed(ABBA),
				AddBlob { blob: bl.clone(), nonce: 10 + 1 },
