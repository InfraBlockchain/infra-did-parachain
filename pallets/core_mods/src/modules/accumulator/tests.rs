...
fn accumulator_events() -> Vec<(super::Event, Vec<H256>)> {
	System::events()
		...
		.filter_map(|event_record| {
			let system::EventRecord::<TestEvent, H256> { phase: _p, event, topics } = event_record;
			match event {
...
fn accumulator_errors() {
	ext().execute_with(|| {
		...
		let add_accum =
			AddAccumulator { id: id.clone(), accumulator: accumulator.clone(), nonce: next_nonce };
		let sig = sign_add_accum(&author_kp, &add_accum, author.clone(), 1);
		assert_err!(
			AccumMod::add_accumulator(RuntimeOrigin::signed(1), add_accum.clone(), sig),
		...
		let ap = AddAccumulatorParams { params: params.clone(), nonce: next_nonce };
		...
		let ak = AddAccumulatorPublicKey { public_key: key.clone(), nonce: next_nonce };
		...
		let add_accum =
			AddAccumulator { id: id.clone(), accumulator: accumulator.clone(), nonce: next_nonce };
		let sig = sign_add_accum(&author_kp, &add_accum, author.clone(), 1);
		AccumMod::add_accumulator(RuntimeOrigin::signed(1), add_accum.clone(), sig.clone())
			.unwrap();
		check_nonce(&author, next_nonce);
		next_nonce += 1;
		...
		let rem_accum = RemoveAccumulator { id: id.clone(), nonce: next_nonce_1 };
		...
		check_nonce(&author_1, next_nonce_1 - 1);
		let rem_accum = RemoveAccumulator { id: id.clone(), nonce: next_nonce };
		...
		assert_err!(
			AccumMod::remove_public_key(RuntimeOrigin::signed(1), rem.clone(), sig),
			Error::<Test>::NotAccumulatorOwner
		);
		check_nonce(&author_1, next_nonce_1 - 1);
		let rem =
			RemoveAccumulatorPublicKey { key_ref: (author.clone(), 1u8.into()), nonce: next_nonce };
		...
		assert_err!(
			AccumMod::remove_params(RuntimeOrigin::signed(1), rem.clone(), sig),
			Error::<Test>::NotAccumulatorOwner
		);
		check_nonce(&author_1, next_nonce_1 - 1);

		let rem =
			RemoveAccumulatorParams { params_ref: (author.clone(), 1u8.into()), nonce: next_nonce };
...
fn add_remove_accumulator() {
	ext().execute_with(|| {
		...
		AccumMod::add_params(
			...
			AddAccumulatorParams { params: params.clone(), nonce: next_nonce },
			sig,
		)
		.unwrap();
		check_nonce(&author, next_nonce);
		next_nonce += 1;
		assert_eq!(AccumulatorParams::get(&author, IncId::from(1u8)), Some(params.clone()));
		assert!(
		...
		AccumMod::add_public_key(
			RuntimeOrigin::signed(1),
			AddAccumulatorPublicKey { public_key: key.clone(), nonce: next_nonce },
			sig,
		)
		.unwrap();
		check_nonce(&author, next_nonce);
		next_nonce += 1;
		assert_eq!(AccumulatorKeys::get(&author, IncId::from(1u8)), Some(key.clone()));
		assert!(
		...
		let add_accum =
			AddAccumulator { id: id.clone(), accumulator: accumulator.clone(), nonce: next_nonce };
		let sig = sign_add_accum(&author_kp, &add_accum, author.clone(), 1);
		AccumMod::add_accumulator(RuntimeOrigin::signed(1), add_accum.clone(), sig).unwrap();
		check_nonce(&author, next_nonce);
		next_nonce += 1;
		assert_eq!(
		...
		let mut rem_accum = RemoveAccumulator { id: id.clone(), nonce: next_nonce - 1 };
		let sig = sign_remove_accum(&author_kp, &rem_accum, author.clone(), 1);
		assert_err!(
			AccumMod::remove_accumulator(RuntimeOrigin::signed(1), rem_accum.clone(), sig),
			sp_runtime::DispatchError::Other("Incorrect nonce")
		...
		let sig = sign_remove_accum(&author_kp, &rem_accum, author.clone(), 1);
		AccumMod::remove_accumulator(RuntimeOrigin::signed(1), rem_accum.clone(), sig).unwrap();
		check_nonce(&author, next_nonce);
