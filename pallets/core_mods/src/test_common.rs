...
pub fn create_did(did: did::Did) -> sr25519::Pair {
	...
	did::Pallet::<Test>::new_onchain(
		...
		did,
		vec![DidKey::new_with_all_relationships(keys_and_sigs::PublicKey::Sr25519(
			util::Bytes32 { value: kp.public().0 },
		))
		.into()],
...
pub fn sign<T: frame_system::Config>(
	...
	SigValue::Sr25519(util::Bytes64 { value: keypair.sign(&payload.encode()).0 })
}
...
pub fn did_sig_on_bytes<T: frame_system::Config, D: Into<Did>>(
	...
	let sig = SigValue::Sr25519(util::Bytes64 { value: keypair.sign(msg_bytes).0 });
